...
int main(void)
{
    ...
    printf("\n\n");
    printf("By How Many Bits Do You Want To Shift psh_a = %d To The Left ?", psh_a);
    scanf("%u", &psh_num_bits);

    printf("\n\n\n\n");
    psh_result = psh_a << psh_num_bits;
    printf("Bitwise LEFT-SHIFT by %d (Decimal) Bits of psh_a =  %d \n gives the Result = %d (Decimal).\n\n", psh_num_bits, psh_a, psh_result);
...
void PrintBinaryFormOfNumber(unsigned int psh_decimal_number)
{
    ...
    printf(
        "The Binary Form Of The Decimal Integer %u Is\t=\t",
        psh_decimal_number
    );
