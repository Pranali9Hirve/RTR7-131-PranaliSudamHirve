...
int main(void)
{
  ...
  {
    if ((psh_num > 0) && (psh_num <= 100)) // 'if' - 02
    {
      printf("Num = %d Is Between 0 And 100 !!!\n\n", psh_num) ;
    }
    else    // 'else' - 02
    {
      if ((psh_num > 100) && (psh_num <= 200)) // 'if' - 03
      {
        printf("Num = %d Is Between 100 And 200 !!!\n\n", psh_num) ;
      }
      else // 'else' - 03
      {
        if ((psh_num > 200) && (psh_num <= 300)) // 'if' - 04
        {
          printf("Num = %d Is Between 200 And 300 !!!\n\n", psh_num) ;
        }
        else // 'else' - 04
        {
          if ((psh_num > 300) && (psh_num <= 400)) // 'if' - 05
          {
            printf("Num = %d Is Between 300 And 400 !!!\n\n", psh_num) ;
          }
          else // 'else' - 05
          {
            if ((psh_num > 400) && (psh_num <= 500)) // 'if' - 06
            {
