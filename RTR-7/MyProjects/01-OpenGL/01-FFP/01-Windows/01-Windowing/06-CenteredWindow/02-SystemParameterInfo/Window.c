...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
    ...
    // Farashi chi width
    int pshScreenWidth = pshRcWorkArea.right - pshRcWorkArea.left;
    // Farashi chi height
    int pshScreenHeight = pshRcWorkArea.bottom - pshRcWorkArea.top;

    // 3: Create the Window
    hwnd = CreateWindow(
        szAppName,
        TEXT("Pranali Hirve: RTR7-131-PranaliSudamHirve/RTR-7/MyProjects/01-OpenGL/01-FFP/01-Windows/01-Windowing/06-CenteredWindow/02-SystemParameterInfo"),
        WS_OVERLAPPEDWINDOW,
        pshRcWorkArea.left + (pshScreenWidth - PSH_WIN_WIDTH) / 2, // x-coordinate, screen 
        pshRcWorkArea.top + (pshScreenHeight - PSH_WIN_HEIGHT) / 2, // y-coordinate
